...
| system-monitor | ~~Rust binary `system-monitor-rs`~~ → **`system-monitor`（去綴）** | ~~ws-sysmon-{weekly,monthly} disabled jobs~~ | ✅ **2026-05-12 已執行接管**（commit `c4dec64f` + manifest 同時清 2 個 disabled job） |
