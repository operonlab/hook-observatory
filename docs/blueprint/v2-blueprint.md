...
### Backend (`services/core/src/core/modules/<name>/`)
...
### Frontend (`apps/web/src/modules/<name>/`)
...
```python
# Backend: services/core/src/core/modules/finance/repository.py
class TransactionRepo(BaseRepository[Transaction]):
    def __init__(self, pool):
        super().__init__(pool, schema="finance", table="transactions")

# Backend: services/core/src/core/modules/finance/services.py
...
# Backend: services/core/src/core/modules/finance/routes.py
...
```typescript
// Frontend: apps/web/src/modules/finance/api.ts
export const transactionApi = createResourceApi<Transaction>('/api/finance/transactions');

// Frontend: apps/web/src/modules/finance/hooks.ts
...
|-------|------------------------|-------------------|
| 1. infra-db | `infra/`, `docker-compose*`, `services/core/migrations/` | - |
| 2. core-engine | `libs/python/`, `services/core/src/core/` (engines, auth, middleware) | `pyproject.toml`, `main.py` |
| 3. domain-modules | `services/core/src/core/modules/{finance,quest,muse,admin}/` | `main.py` (router registration) |
| 4. web-complete | `apps/web/`, `libs/typescript/` | `package.json` |
