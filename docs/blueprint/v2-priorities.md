...
| 順位 | 模組 | 目標 | 為什麼優先 |
|------|------|------|-----------|
| **P1** | memory (KAS Memory V2) | Claude Code 持久化記憶 | 每天都在用，改善記憶品質 = 改善所有工作品質 |
| **P2** | intel (Smart Search V2) | 搜尋報告結構化儲存 + UI | 搜尋是高頻操作，散落 .md 檔已造成痛點 |

---

## P1：Memory 模組 — KAS Memory V2
...
現況：memories/*.md + tags.idx + embeddings.json
  ↓
目標：PostgreSQL (schema: memory)
  ├── memory_blocks     — 記憶區塊（取代 .md 檔案）
  ├── memory_tags       — 標籤索引（取代 tags.idx）
  ├── memory_embeddings — 向量欄位（取代 embeddings.json，pgvector）
...
**Galaxy 視覺化升級**：從靜態 HTML 升級為 Dashboard Widget，即時反映最新狀態。
...
```
dashboard/src/modules/memory/     ← Memory UI（Galaxy Widget、記憶瀏覽器）
core/src/modules/memory/          ← Memory 後端（API + DB + 提煉引擎）
mcp/memory/                       ← MCP Adapter（保持現有 9 個工具介面）
```

### 遷移策略

1. **Phase A**：在 Core 中建立 memory schema + API，匯入現有 .md 記憶
2. **Phase B**：切換 MCP Server 為 Core API 的薄適配器（而非直接讀檔案）
3. **Phase C**：升級 SessionEnd Hook 寫入 DB 而非 .md
4. **Phase D**：Dashboard Widget（Galaxy + 記憶瀏覽器）

---

## P2：Intel 模組 — Smart Search V2
...
將散落的搜尋/情報能力整合為 Workshop Core 的 `intel` 模組。
...
現況：
  ...
  ↓
目標：
  core/src/modules/intel/             (Core Module)
  ├── schema: intel                   (PostgreSQL)
  │   ├── reports          — 搜尋/研究報告
...
smart-search Skill (Claude Code)
    │
    ├── Pre-Search: POST /api/intel/search/check  ← 查重（pgvector 相似度）
    │
    ├── 執行搜尋（DeepWiki / Context7 / Perplexity / 9 平台社群）
    │
    └── Post-Search: POST /api/intel/reports      ← 寫入報告 + 自動 embedding
                                                     不再 fallback 到 .md
```

#### 3. UI 升級：Research Hub → Dashboard Module

保留 V1 的好設計，以 Dashboard 模組形式重建：

| 頁面 | 功能 |
|------|------|
| `/intel` | 情報總覽（最新報告、趨勢圖、主題圖譜） |
| `/intel/reports` | 報告列表（全文搜尋 + 語意搜尋 + tag 過濾） |
| `/intel/reports/:id` | 報告詳情（Markdown 渲染 + 來源連結） |
| `/intel/topics` | 主題圖譜（Force-directed graph 視覺化） |
| `/intel/briefings` | 每日情報（三分析師辯論格式） |
| Widget | Dashboard 首頁摘要 Widget（最新 5 篇 + 趨勢） |

#### 4. Daily Briefing 整合

V1 的三 AI 辯論模式整合到 intel 模組：
...
每日觸發（cron 或手動）
    ...
    │
    └── 寫入 DB：POST /api/intel/briefings
...
```
dashboard/src/modules/intel/      ← Intel UI（報告瀏覽、主題圖譜、情報總覽）
core/src/modules/intel/           ← Intel 後端（API + DB + pgvector + 提煉）
mcp/intel/                        ← MCP Adapter（供 Claude Code 直接操作）
```

### 遷移策略

1. **Phase A**：在 Core 中建立 intel schema，匯入 research_report DB 資料 + 52 個 .md 回填
2. **Phase B**：切換 smart-search Skill 端點從 `localhost:8830` → Core API
3. **Phase C**：Dashboard 模組（報告瀏覽器 + 主題圖譜）
