...
|------|------------|-------------|
| `admin` | `*` (全部) | 全平台存取權限 |
| `user` | `finance.read`, `finance.write`, `quest.read`, `quest.write`, `muse.read`, `muse.write` | 標準啟用使用者 |
| `guest` | `finance.read`, `quest.read`, `muse.read` | 唯讀存取權限 |
...
範例：
  ...
  quest.read          查看任務 (quests)
  quest.write         建立/管理任務 (quests)
  admin.users         管理使用者
...
插件運行時的權限為其宣告權限與當前使用者權限的**交集 (intersection)**：

```
effective_permissions = plugin.manifest.permissions ∩ user.permissions
```

對於 admin 使用者，宣告 `["finance.read", "quest.write"]` 的插件將同時獲得這兩項權限。對於 guest 使用者，該插件僅能獲得 `finance.read`（因為 guest 缺乏 `quest.write`）。

參閱 [Plugin System](./plugin-system.md) 以瞭解 manifest 格式。
