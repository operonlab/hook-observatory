...
> **Status**: ⚠️ 大部分完成 (2026-04-05 更新)
> **Outcome**: Wave 1-3 全部完成，invest CLI+MCP 已補齊，taskflow 已有完整模組。剩餘：ideagraph 仍為骨架（缺 models/services），anvil onboarding 未補。
>
> _2026-03-07 原始備註：Wave 1-3 的 SDK 層已全部完成（20+ clients），大部分 MCP 和 CLI 也已到位。_
...
**A. 核心模組 SDK** — 繼承 `BaseClient`（包裝 `/api/{module}/` 端口 10000）：
```python
