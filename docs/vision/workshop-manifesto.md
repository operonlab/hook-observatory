...
| **muse** | 靈感筆記、知識圖譜、收件匣 |
| **scout** | 每日情報、自動摘要、RSS/社交監測 |
| **lore** | KAS Memory v2 (LLM 記憶持久化) |
| **dojo** | 技能樹、學習路徑、能力驗證 |
| **roster** | 資源管理 (人力/機器/服務/AI agent 的統一抽象) |
| **nexus** | 媒合引擎 (人才×工作，能力×任務) |
| **admin** | 平台管理、系統監控、配置 |
...
├─────────────────────────────────────────────┤
│  Layer 1: 模組 SPA 頁面                      │ ← /finance/*, /quest/* ...
│  每個模組有完整的路由與 UI                     │
└─────────────────────────────────────────────┘
```
