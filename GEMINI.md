...
# Core Project Independence 20260511

---
name: core 專案獨立性與技術策略 2026-05-11
description: story-entity-model 從 workshop 拉出為獨立大專案；整體代號 core；開發 py+js→穩定後 rust(主)/go(次) 1:1 重構
type: project
originSessionId: 099fac3d-997a-4fa9-a7f8-26e1ee5bd2f8
---
# core 專案獨立性與技術策略（2026-05-11）

原 `outputs/story-entity-model/` 的生態系藍圖，少爺 2026-05-11 拍板：**不放 workshop，是獨立大專案**。

## 1. 專案性質：完全獨立（A1）

- 獨立 repo（不是 workshop monorepo 子目錄）
- 獨立 deploy（不寄生在 workshop core 服務）
- 獨立 domain（未來可開源 / 給別人用）
- 跟 workshop 完全解耦（不依賴 workshop auth / db / observability / port_registry）

**目前狀態**：藍圖文件還在 `~/workshop/outputs/story-entity-model/`，未來會拉出去。拉出去時機與 repo 名待定。

## 2. 命名：整體先用 core

- `core` 是整個專案的暫定代號（之後可能改名）
- **不是** workshop 內某個 module 的名字
- 不是子 module 名（如 `ledger`/`nexus`/`ecosystem`/`playbook`）全部作廢
- 因此 03-entity-relation/ 的 8 概念 + 10 entity 對應整個 `core` 專案的核心領域

## 3. 技術選型策略：LLM-first（C）

### 開發階段（v1 起）：Python + JavaScript

- 理由：LLM 對這兩個語言正確率最高、社群最大
- 雙語言並行可能（具體分工待定）
- 純 py-only 功能保留 py，純 js-only 功能保留 js

### 穩定後：1:1 重構到 Rust（主）/ Go（次）

- 重構不換語意（同一份 entity 模型、同一份 service contract）
- Rust 為主軸（效能 + 安全）
- Go 為次要（簡單 service / 部署便利處）

### 底層理念

- LLM 寫程式 + 給足 token = 時間不是限制
- 語言選擇優先看 **LLM 正確率 + 社群規模**，不優先看效能
- 效能在穩定後用 Rust 補

## 4. 對先前決策的影響

- 激進派 agent 推的 XTDB（JVM/Clojure 主場）**不直接適合 py/js 階段**——若要走 event-sourcing 路線，需挑 py/js 友好 + rust 重構期也順利的 store
- 保守派 agent 推的 FastAPI + SQLAlchemy 不再是預設，但 Python + FastAPI 仍是候選之一
- 模組命名候選（ledger / nexus / ecosystem / playbook）**全部作廢**——專案代號就叫 `core`
- 舊版 `story-entity-model-state-20260506.md` 中「module 名候選」段落作廢

## 5. 後續決議（2026-05-11 全部拍板）

### 5.1 語言分工

- **前端：JavaScript**
- **後端：Python**
- v1 粗略分配，更細的拆法（FFI / 邊界 service）視實作展開

### 5.2 重構路線（始終記住）

最終形態 = **Rust（主）/ Go（次）為主，Python/JS 為輔**。py/js 階段是 prototype + LLM 友善期；穩定後 90%+ 1:1 重構到 rust/go，例外保留 py-only（如 AI/ML 生態）或 js-only 功能。

### 5.3 架構模式：最新狀態 + 歷史事件，事件驅動

**不走純 Event Sourcing**（從 events 重建 state），而是雙軌：

- **業務 entity 表存當下狀態**（actors / contexts / commitments / works / resources / rules / representations 等）—— 直接 query 拿到「現在是什麼」
- **history_events 存歷史**（append-only 事件流）—— 追溯「曾經發生過什麼」、跨系統通知、audit
- **寫入路徑**：每次寫業務 entity 必同時寫一條 history_events（一致性保證留給工程階段選機制）
- **事件流可訂閱**：其他 module / agent / 外部系統可訂閱 history_events 做即時反應

這跟 04-v1-entity-map.md 目前設計完全吻合，是「快照 + 事件」雙軌制，不是 ES 也不是純 CRUD。

### 5.4 API 形式

- **REST 為主**（覆蓋 80%+ 場景）
- **gRPC** 用於性能熱路徑（內部 service 間高頻調用）
- **WebRTC** 用於即時通訊場景（如有）
- 其他協定視情況加入（GraphQL 不預設用，除非有強需求）

### 5.5 獨立 repo 即將創建（2026-05-11 授權）

少爺已授權創建獨立 repo。位置、目錄結構、藍圖文件搬運待 Claude 提案後動手。

## 6. 進工程實作前已 clear 的事

- entity 層 4 個破洞已修補（2026-05-11，見 04-v1-entity-map.md）
- 3 個可疑點已處理
- 缺口分類調整完成（6 必 + 4 應 + 5 接縫 + 1 不做）
- representations 升「必」
- visibility 雙模式拍板
- 三軸親簽歷史走 history_events
- self_attest 留 commitments + status 限制
...
description: Session Channel 朝 blog tmux-as-bridge 願景的進化 (Phase 1-3f) — cross-agent statline + UserPromptSubmit inbox + push notify + handoff + trust marker classifier fix
type: project
...
- Claude session 中途 `/exit` 機制不明（觀察到多次 spontaneous exit，可能 user 切過去手動退、可能 idle 機制）—— worker pool 需要 supervised respawn
- auto-mode classifier 是 ML 判斷，trust marker 是 hint，不保證 100% allow（bypass mode 才是最徹底解）
- inbox auto-inject 對 user prompt 是技術命令（如「`channel read tasks`」）時，Claude 視為 user ask 處理完就停，不進入 worker mode — Phase 3c push 直推 prompt 本身才解掉這層
- Codex / Gemini adapter 還沒寫（schema 預留 `cli` 欄位 future-proof）

## 待辦（v0.4+）

- Codex / Gemini adapter wrapper
- Worker auto respawn supervisor（偵測 /exit 後自動 claude --resume）
- Tasks topic 失敗回報 + retry 政策
- Cross-host pane 真實驗證（_meta.host 已預留）

## Critical Files

- `stations/session-channel/cli/channel.py` — CLI (send/read/agents/topics/health) + `_tmux_nudge` push 邏輯 + trust marker
- `stations/session-channel/routes.py` — `_meta` 透傳 + `/api/agents/active` reduce + read order param
- `stations/session-channel/templates/index.html` — agent-card render + LOCAL_KEY 注入 + SSE
- `stations/hook-dispatcher/internal/handlers/session_channel.go` — SessionStart/PreToolUse/Stop/SessionEnd/UserPromptSubmit handler
