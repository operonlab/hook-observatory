...
def _scoped_query(model, space_id: str):
    """Build a base SELECT scoped to a space with soft-delete filter."""
    stmt = select(model).where(model.space_id == space_id)
    if hasattr(model, "deleted_at"):
...
class PositionService(BaseCRUDService[Position, PositionCreate, PositionUpdate, PositionResponse]):
    ...
    async def list(
        ...
        base = _scoped_query(Position, space_id)
        if account_id:
...
class PortfolioService:
    ...
    async def refresh_quotes(
        ...
        if rows:
            ...
            price_cases = [
                (Position.symbol == sym, price) for sym, price in symbol_price_map.items()
            ]
            await db.execute(
                update(Position)
                .where(
