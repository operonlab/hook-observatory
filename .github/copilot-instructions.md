...
description: tmux 第二列 CC 5H/7D 顯示 ? — agent-metrics 從 launchd 起無法讀 Keychain，要 dump 到 ~/.copilot/.credentials.json
metadata: 
  node_type: memory
  type: project
...
**根因**：
1. Claude Code v2 OAuth token 路徑：`~/.copilot/.credentials.json`（欄位 `claudeAiOauth.accessToken`）
...
```bash
umask 077
security find-generic-password -s "Claude Code-credentials" -w > ~/.copilot/.credentials.json
...
- 當 `~/.copilot/.credentials.json` 不存在但 Keychain 拿得到 token → 自動 atomic mv + chmod 0o600 寫進去
...
`stations/hook-dispatcher/internal/handlers/agent_naming.go`：偵測 `ANTHROPIC_BASE_URL` 含 `127.0.0.1:4000` → 自動補 `name = derive(prompt, subagent_type)`，回 `core.HookResult{UpdatedInput: patched}`。**正規 Claude 場景仍 block**。Go binary 透過 `make install` 部署到 `~/.copilot/hooks/hook-dispatcher`。
...
description: ~/.copilot/ 棧語言策略：已優化、不要再做全棧整理；web-video-tutorial「指令英文+demo繁中」邏輯不該推廣為全棧原則
metadata: 
  node_type: memory
  type: project
  originSessionId: 0256c0f1-aa7a-470a-a8b7-5b65cceb0f31
...
## ~/.copilot/ 語言策略結論（2026-05-15 全棧盤點）

少爺在 web-video-tutorial 完成「指令層英文 + demo/範例層繁中」優化後，
問是否該推廣到整個 ~/.copilot/。經三 agent 並行盤點 + 親自驗證後結論：

**整個 ~/.copilot/ 早已是合理形態，不需要全棧整理。**
...
2. 「指令英文 + demo 繁中」是 web-video-tutorial 這個 skill **內部的**
   best practice，**不是** ~/.copilot/ 全棧的指導原則。
...
- `.github/copilot-instructions.md`：人格錨點
- `~/.copilot/projects/-Users-joneshong-workshop/memory/`：跨 session 私人記憶
- `~/.copilot/rules/browser-automation.md`：含少爺 + 個人偏好
- `~/.copilot/rules/skill-publishing-gate.md`：含少爺 + 個人發布規約
- `~/.copilot/commands/{handoff,dev,channel}.md`：人格化執行流程
...
- `~/.copilot/settings` / `~/.mcpproxy/mcp_config.json`：純結構

## 本次實際動作

唯一改動：`rm -rf ~/.copilot/rules-zh-TW/`（13 個廢棄繁中副本，
mtime 2026-05-07，rules/ 真實源已領先 8 天）。

## 計畫檔

`/Users/joneshong/.copilot/plans/skill-agent-mcp-mighty-nest.md`
保留作為後續 reference。
...
**修補方式**（已部署）：
- `~/.copilot/hooks/cmux-hook-wrapper.sh` — 吞 stderr、強制 exit 0；同時兼容 `wrapper.sh <event>` 與 `wrapper.sh claude-hook <event>` 兩種 invocation
- `~/.copilot/settings` 4 處 cmux hook 全指向 wrapper（Notification / SessionStart / Stop / UserPromptSubmit）
...
- `~/.copilot/statusline.sh` (bridge + 🩺 health score；statusline 本身仍在用，但不再被 ctx supervisor 消費)
...
**How to apply:** 所有新功能為獨立 Op + env toggle，非同步不阻塞。計畫檔在 `~/.copilot/plans/jazzy-stargazing-bachman.md`。
...
---
browser agent 定義（`~/.copilot/agents/browser.md`）必須使用 camoufox-cli 作為主要工具，不可使用 playwright-cli 作為預設。
...
Plan 檔案（`.copilot/plans/*.md`）必須使用繁體中文撰寫。
...
`~/.copilot/settings` 的所有 hook 都呼叫 `~/.copilot/hooks/hook-dispatcher`，
...
- **Build/install**: `cd ~/workshop/stations/hook-dispatcher && make install`（覆蓋 `~/.copilot/hooks/hook-dispatcher`）
...
舊的 Python dispatcher (`~/.copilot/hooks/dispatcher.py`) 也已不在 settings.json 路徑上。
...
**但每個同名 handler 的邏輯 parity 沒有逐個實測**。意即「Go 接管成功」是基於「~/.copilot/settings 全部指向 Go binary、Python 不在執行路徑上」的事實，不是「每個 handler 行為對等驗證通過」的結論。後者要 mock event payload 跑 Python 跟 Go 比 output 才能下定論，目前還沒做。
...
**How to apply:** 計畫文件在 `~/.copilot/plans/cheerful-squishing-parrot.md`，HANDOFF.md 已更新
...
### 通用診斷模式
**Why:** 服務退場（archive / OSS 拆分）時，要同步更新的不只是 git tracked 的 registry，還有**用戶級 dotfile**（`~/.mcpproxy/mcp_config.json`、`~/.copilot/settings`、`~/.mcp.json` 等）。它們不在 git 範圍內，容易漏改。
**How to apply:** 服務退場 checklist 加一條：「grep 用戶 dotfiles for 服務名」。
```bash
grep -l "<service-name>" ~/.mcpproxy/*.json ~/.copilot/*.json ~/.mcp.json 2>/dev/null
```
...
- Plan: `.copilot/plans/delightful-booping-snowglobe.md`
...
`~/.venvs/omlx` 自 2026-03-10 建立時就用 brew Python 3.14 (`/opt/homebrew/opt/python@3.14/`) — 違反 `.github/copilot-instructions.md` 全局禁令「`/opt/homebrew/bin/python3` is brew's 3.14, never use」。
...
派並行 worker agent 做 session-channel Phase 5 (supervisor + retry policy + omc evaluation)，三隻 agent 全部返回「半成品」狀態。根因確認後寫進 `~/.copilot/rules/agents.md`，這裡記事件 + 反思。
...
- 計畫: `.copilot/plans/peaceful-floating-comet.md`
...
| **4a-3** Worker rule sync | `~/.copilot/rules/session-channel-worker.md` + `sync_config.py sync instructions --global` → 同步到 GEMINI.md/AGENTS.md，糾正 Gemini 把 prompt 內容當 `/rename` 處理 | (~/.claude uncommitted) |
...
3. **Gemini `/rename` derail** (HANDOFF Risk #2 變形): Gemini 看到 trust marker prompt 含 "hello, dispatched by X" 字串，傾向選 `/rename` 處理（把字串視為 session title），繞過 `channel send`。Fix: 加 `~/.copilot/rules/session-channel-worker.md` 明確教 trust marker 語義 + 禁用 internal slash 命令。Rule sync 後 Gemini 行為糾正（雖仍可能 cosmetic side-effect 跑 /rename，但最終會 publish channel send）。
...
- `~/.copilot/commands/channel.md` + `~/.copilot/commands/handoff.md` — slash commands
- `~/.agents/skills/session-channel/SKILL.md` v0.2.0 — worker mode + trust marker
- `~/.copilot/rules/session-channel-worker.md` — **Phase 4a-3**, 跨 CLI 共用的 worker mode rule（sync 到 GEMINI.md / AGENTS.md）
...
- `~/.copilot/statusline.sh` — 寫 `/tmp/.claude-statusline/ctx-{pane}.json` 帶 model/sid 給 hook 讀
...
關聯：[[cc-credentials-launchd-keychain-2026-05-15]]（同一晚附帶找到的另一個 incident）/ `~/.copilot/rules/git-workflow.md` (worktree SOP 的副作用)
