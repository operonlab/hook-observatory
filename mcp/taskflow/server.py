...
def fmt_task_line(t: dict) -> str:
    ...
    s_emoji = TASKFLOW_STATUS_EMOJI.get(status, "❓")
    p_emoji = TASKFLOW_PRIORITY_EMOJI.get(priority, "")
    due = t.get("due_date", "")
...
def fmt_task_detail(t: dict) -> str:
    ...
    lines = [
        f"# {t.get('title', '(無標題)')}",
        "",
        f"- **狀態**: {TASKFLOW_STATUS_EMOJI.get(status, '')} {status}",
        f"- **優先級**: {TASKFLOW_PRIORITY_EMOJI.get(priority, '')} {priority}",
        f"- **來源**: {t.get('source', '')}",
...
async def taskflow_create_task(
    ...
    return (
        f"任務已建立\n"
        f"- 標題: {result['title']}\n"
        f"- 優先級: {TASKFLOW_PRIORITY_EMOJI.get(p, '')} {p}\n"
        f"- ID: `{result['id'][:8]}`"
    )
...
async def taskflow_add_update(
    ...
    body = build_body({"type": type, "content": content}, hours_spent=hours_spent)
...
async def taskflow_progress() -> str:
    ...
    if by_status:
        ...
        for status, count in by_status.items():
            emoji = TASKFLOW_STATUS_EMOJI.get(status, "")
    ...
    if by_priority:
        ...
        for priority, count in by_priority.items():
            emoji = TASKFLOW_PRIORITY_EMOJI.get(priority, "")
