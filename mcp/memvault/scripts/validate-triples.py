...
PREDICATE_ALIASES = {
    ...
    "complements": "enables",
    "type": "format_is",
    "version": "configured_with",
    "updated with": "configured_with",
    "replaced_by": "chosen_over",
    "replaces": "chosen_over",
    "is_a": "maps_to",
    "has": "configured_with",
    "related_to": "enables",
    "affects": "causes",
    ...
    "generates": "enables",
    "validates": "fixes",
    "implements": "implemented_as",
    "contains": "configured_with",
    "provides": "enables",
    "disables": "prevents",
    "overrides": "configured_with",
    "inherits": "depends_on",
    "returns": "flow_is",
    "scans": "uses",
    "suitable_for": "should",
    ...
    "handles": "flow_is",
    "parses": "uses",
    "stores": "configured_with",
    "caches": "configured_with",
    "logs": "uses",
    "monitors": "uses",
    "connects_to": "depends_on",
    "listens_on": "configured_with",
    "exposes": "enables",
    "serves": "enables",
    "integrates_with": "depends_on",
    "built_with": "uses",
    "optimizes": "improves",
    ...
    "ensures": "should",
    "enforces": "should",
    "retains": "configured_with",
    "released_with": "configured_with",
    "is": "format_is",
    "focus": "pattern_is",
    "produces": "causes",
    "outputs": "causes",
    ...
    "sets": "configured_with",
    "defines": "format_is",
    "specifies": "format_is",
    "limits": "configured_with",
    "restricts": "prevents",
    "allows": "enables",
    "permits": "enables",
    "requires_not": "should_NOT",
    "must_not": "should_NOT",
    "depends": "depends_on",
    "relies_on": "depends_on",
    "based_on": "depends_on",
    "derived_from": "depends_on",
    "replicated_from": "maps_to",
    "mirrors": "maps_to",
    ...
    "reduces": "improves",
    "minimizes": "improves",
    "includes": "configured_with",
    "constrained_by": "requires",
    "ignores": "should_NOT",
    "updated_with": "configured_with",
    "lacks": "should",
    "missing": "should",
    "conflicts": "prevents",
    "compatible_with": "enables",
    "incompatible_with": "prevents",
    "deprecated": "should_NOT",
    "replaces_with": "chosen_over",
    "migrated_to": "chosen_over",
    "wraps_around": "implemented_as",
    "abstracts": "implemented_as",
    "delegates_to": "depends_on",
    "proxies": "depends_on",
    "notifies": "causes",
    "signals": "causes",
    "listens_to": "depends_on",
    "watches": "uses",
    "spawns": "causes",
    "terminates": "prevents",
    "persists": "configured_with",
    "caches_in": "configured_with",
}
...
def validate(data):
    ...
    # 2026-05-08：unknown category 改 warn + fallback "preference"，避免一個
    # LLM hallucinated category (e.g. "writing_style") 讓整個 session validation 失敗
    unknown_cat_count = 0
    for i, c in enumerate(data["corrections"]):
        ...
        if cat and cat not in CORRECTION_CATEGORIES:
            unknown_cat_count += 1
            c["category"] = "preference"  # graceful fallback
    if unknown_cat_count:
        warn(f"Normalized {unknown_cat_count} unknown correction category to 'preference'")

    # Soft warnings
    n = len(data["triples"])
    if n == 0:
        warn('triples is empty — consider {"skip": true}')
    if n > 12:
        warn(f"{n} triples (recommended max: 12)")
    t = len(data["tags"])
    if t < 3:
        warn(f"Only {t} tag(s) — min 3 recommended")
    elif t > 8:
        warn(f"{t} tags — max 8 recommended")
