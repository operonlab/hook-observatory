    ...
    <div className="finance flex flex-col md:flex-row h-full">
      ...
      >
        ...
        >
          ...
          <span className="text-sm font-medium tracking-wide" style={{ color: 'var(--fn-accent)' }}>
            記帳理財
          </span>
        </div>
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <NavLink
                ...
                style={{
                  backgroundColor: active ? 'var(--fn-accent-alpha)' : 'transparent',
                  color: active ? 'var(--fn-accent)' : 'var(--fn-text-tertiary)',
                  borderLeft: active ? '2px solid var(--fn-accent)' : '2px solid transparent',
                }}
              >
                <Icon size={15} />
                {item.label}
