...
export const transactionApi = {
	...createCrudApi<Transaction, TransactionCreate, TransactionUpdate>(
...
export const subscriptionApi = createCrudApi<
	Subscription,
	SubscriptionCreate,
	SubscriptionUpdate
...
export const installmentApi = {
	...
	cancel: (id: string) =>
		request<void>(`/finance/installments/${id}/cancel`, { method: "POST" }),

	payoff: (id: string) =>
		request<void>(`/finance/installments/${id}/payoff`, { method: "POST" }),
};

export const budgetApi = {
	list: (yearMonth: string) =>
		request<Budget[]>(`/finance/budgets?year_month=${yearMonth}`),
...
export const analyticsApi = {
	...
	insights: (months = 6) =>
		request<MonthlyTrend[]>(`/finance/insights?months=${months}`),

	categoryBreakdown: (yearMonth: string) =>
		request<CategoryBreakdown[]>(
			`/finance/summary?year_month=${yearMonth}`,
		).then((s) => (s as unknown as MonthlySummary).by_category),
