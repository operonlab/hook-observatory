...
const reportCrud = createCrudApi<Report, ReportCreate, ReportUpdate>(
  "/intelflow/reports",
);
...
export const intelflowApi = {
  ...
  getDashboard: () =>
    request<DashboardData>("/intelflow/dashboard"),

  getTimeline: (days = 30) =>
    request<{ entries: TimelineEntry[] }>(
      `/intelflow/dashboard/timeline?days=${days}`,
    ),

  listTopics: (page = 1, pageSize = 50) =>
    request<PaginatedResponse<Topic>>(
      `/intelflow/topics?page=${page}&page_size=${pageSize}`,
    ),

  getTopicGraph: () =>
    request<TopicGraph>("/intelflow/topics/graph"),
  ...
  addBriefingSubtopic: (topicId: string, data: BriefingSubtopicCreate) =>
    request<BriefingSubtopic>(
      `/intelflow/briefings/topics/${topicId}/subtopics`,
      { method: "POST", body: JSON.stringify(data) },
    ),

  updateBriefingSubtopic: (
    topicId: string,
    subtopicId: string,
    data: BriefingSubtopicUpdate,
  ) =>
    request<BriefingSubtopic>(
      `/intelflow/briefings/topics/${topicId}/subtopics/${subtopicId}`,
