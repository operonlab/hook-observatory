#!/usr/bin/env bash
# Sync Claude Code configs to Qwen Code
# Usage: ./sync-qwen-config.sh

set -e

WORKSHOP_ROOT="/Users/joneshong/workshop"
CLAUDE_RULES="$WORKSHOP_ROOT/.claude/rules"
QWEN_RULES="$WORKSHOP_ROOT/.qwen/rules"

echo "🔄 Syncing Claude Code configs to Qwen Code..."

# Ensure .qwen directories exist
mkdir -p "$QWEN_RULES"
...
for rule_file in "$CLAUDE_RULES"/*.md; do
    if [ -f "$rule_file" ]; then
        ...
        echo "  → $filename"
        cp "$rule_file" "$QWEN_RULES/"
    fi
done

# Add Workshop main rule (always last, overrides if needed)
echo "  → workshop.md (main rules)"
cat > "$QWEN_RULES/workshop.md" << 'EOF'
# Workshop 專案規則
...
# Generate MCP config (using ProxyMCP like Claude Code)
echo "🔌 Generating MCP config (ProxyMCP)..."
cat > "$WORKSHOP_ROOT/.qwen/mcp.json" << 'EOF'
{
  "mcpServers": {
    "mcpproxy": {
      "type": "http",
...
echo "✅ Sync complete!"
echo ""
echo "📁 Files synced to: $QWEN_RULES"
echo "🔌 MCP config: $WORKSHOP_ROOT/.qwen/mcp.json"
echo ""
echo "💡 Skills are shared via ~/.agents/skills/"
