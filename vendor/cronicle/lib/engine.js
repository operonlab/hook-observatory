...
module.exports = Class.create({
	...
	tick: function () {
		...
		if (this.numSocketClients) {
			...
			if (this.multi.manager) {
				...
				// but keep some props that might be used by UI
				
				let activeJobs = JSON.parse(JSON.stringify(this.getAllActiveJobs(true)));
				for (let id in activeJobs) { 
					if(activeJobs[id].params) {
						let jp = activeJobs[id].params || {}
						activeJobs[id].params = {}
						if(jp.cols) activeJobs[id].params['cols'] = jp.cols
						if(jp.rows) activeJobs[id].params['rows'] = jp.rows
						if(jp.lang) activeJobs[id].params['lang'] = jp.lang
					}						
				...
				status.platform = os.platform() || 'Unknown';
				status.release = os.release() || 'Unknown';
			}
	...
	updateClientData: function (name) {
		...
		this.storage.listGet('global/' + name, 0, 0, function (err, items) {
			...
			let itemCopy = items ? JSON.parse(JSON.stringify(items)) : [] // make a copy to avoid side effects
			// clear secret info
	...
	emergencyShutdown: function (err) {
		...
		let webhook = this.server.config.get('admin_web_hook');
		if (webhook) {
			try { cp.execSync(`curl -X POST -H 'Content-Type: application/json' -d '{"text":"${errorMsg}"}' ${webhook}`); }
			catch(e) { this.logError('webhook', "Failed to send webhook: " + e.message); }
		}
		this.abortAllLocalJobs();
	},
