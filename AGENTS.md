...
`stations/hook-dispatcher/internal/handlers/agent_naming.go`：偵測 `ANTHROPIC_BASE_URL` 含 `127.0.0.1:4000` → 自動補 `name = derive(prompt, subagent_type)`，回 `core.HookResult{UpdatedInput: patched}`。**正規 Claude 場景仍 block**。Go binary 透過 `make install` 部署到 `~/.codex/hooks/hook-dispatcher`。
...
**修補方式**（已部署）：
- `~/.codex/hooks/cmux-hook-wrapper.sh` — 吞 stderr、強制 exit 0；同時兼容 `wrapper.sh <event>` 與 `wrapper.sh claude-hook <event>` 兩種 invocation
- `~/.codex/config.toml` 4 處 cmux hook 全指向 wrapper（Notification / SessionStart / Stop / UserPromptSubmit）
...
- `~/.codex/statusline.sh` (bridge + 🩺 health score)
...
**How to apply:** 所有新功能為獨立 Op + env toggle，非同步不阻塞。計畫檔在 `~/.codex/plans/jazzy-stargazing-bachman.md`。
...
---
browser agent 定義（`~/.codex/agents/browser.md`）必須使用 camoufox-cli 作為主要工具，不可使用 playwright-cli 作為預設。
...
Plan 檔案（`.codex/plans/*.md`）必須使用繁體中文撰寫。
...
`~/.codex/config.toml` 的所有 hook 都呼叫 `~/.codex/hooks/hook-dispatcher`，
...
- **Build/install**: `cd ~/workshop/stations/hook-dispatcher && make install`（覆蓋 `~/.codex/hooks/hook-dispatcher`）
...
舊的 Python dispatcher (`~/.codex/hooks/dispatcher.py`) 也已不在 settings.json 路徑上。
...
**How to apply:** 計畫文件在 `~/.codex/plans/cheerful-squishing-parrot.md`，HANDOFF.md 已更新
...
- Plan: `.codex/plans/delightful-booping-snowglobe.md`
...
- 計畫: `.codex/plans/peaceful-floating-comet.md`
