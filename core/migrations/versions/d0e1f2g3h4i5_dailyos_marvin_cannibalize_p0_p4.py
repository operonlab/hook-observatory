...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_ut_unique_key", "user_toggles", ["space_id", "toggle_key"],
    ...
    op.create_table(
        ...
        sa.Column("parent_id", sa.String(32), sa.ForeignKey(f"{SCHEMA}.backlog_items.id", ondelete="SET NULL"), nullable=True),
    ...
    op.create_index("idx_bi_space_layer", "backlog_items", ["space_id", "funnel_layer"], schema=SCHEMA)
    ...
    op.create_index(
        "idx_ch_unique_date", "capacity_history", ["space_id", "log_date", "budget_type"],
        unique=True, schema=SCHEMA,
        postgresql_where=sa.text("deleted_at IS NULL"),
    )
    ...
    op.create_index(
        "idx_wf_unique_slug", "workflows", ["space_id", "slug"],
        unique=True, schema=SCHEMA,
        postgresql_where=sa.text("deleted_at IS NULL"),
    )
    ...
    op.create_index(
        "idx_sn_unique_slug", "snippets", ["space_id", "slug"],
        unique=True, schema=SCHEMA,
        postgresql_where=sa.text("deleted_at IS NULL"),
    )
    ...
    op.create_table(
        ...
        sa.Column("icon", sa.Text, nullable=True),
        sa.Column("color", sa.Text, nullable=True),
        sa.Column("filter_expr", postgresql.JSONB, nullable=False, server_default=sa.text("'[]'::jsonb")),
    ...
    op.create_index(
        "idx_sl_unique_slug", "smart_lists", ["space_id", "slug"],
        unique=True, schema=SCHEMA,
        postgresql_where=sa.text("deleted_at IS NULL"),
    )
    ...
    op.create_index(
        "idx_ps_unique_date", "pilot_state", ["space_id", "state_date"],
        unique=True, schema=SCHEMA,
        postgresql_where=sa.text("deleted_at IS NULL"),
    )
    ...
    op.create_index(
        "idx_pt_unique_slug", "plan_templates", ["space_id", "slug"],
        unique=True, schema=SCHEMA,
        postgresql_where=sa.text("deleted_at IS NULL"),
    )
    ...
    op.create_index(
        "idx_gs_unique_space", "gamification_state", ["space_id"],
        unique=True, schema=SCHEMA,
        postgresql_where=sa.text("deleted_at IS NULL"),
    )
    ...
    op.create_table(
        ...
        sa.Column("variant_a", postgresql.JSONB, nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("variant_b", postgresql.JSONB, nullable=False, server_default=sa.text("'{}'::jsonb")),
