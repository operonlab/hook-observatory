...
CREATE TABLE intelflow.briefings (
    id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    date        DATE NOT NULL,
    topic_id    UUID REFERENCES intelflow.briefing_topics(id), -- 關聯動態主題（取代寫死 domain）
...
| 頁面 | 路徑 | 功能 |
|------|------|------|
| 主題管理 | `/intelflow/briefings/settings` | CRUD 主題 + 子分類，排程設定 |
| 情報瀏覽 | `/intelflow/briefings` | 依日期檢視各主題情報 |
| 情報詳情 | `/intelflow/briefings/:date/:topic` | 三分析師分析 + 辯論 |
...
1. 建立 schema + models → 匯入現有 `workshop_research` DB 資料
...
6. 退役 V1 `~/Claude/services/research_report/`

## 相依模組

- **auth** — space_id 隔離
- **memvault** — 搜尋報告可觸發記憶建立（跨模組事件）
- **mcp/intelflow** — MCP 工具對接

## Skill 整合

除了 smart-search 和 daily-briefing 外，以下 Skills 的產出也統一存入 intelflow DB：

| Skill | 整合方式 |
|-------|---------|
| **company-intel** | 報告存入 `intelflow.reports`（tag: company-intel） |
| **competitive-intel** | 報告存入 `intelflow.reports`（tag: competitive-intel） |
| **content-writer** | 文章存入 `intelflow.reports`（tag: content-article），來源連結存入 `sources` JSONB |

所有研究成果統一入庫後，可跨 skill 語意搜尋，避免資訊孤島。

## 參考

- V1 research_report（已遷移）：`~/Claude/services/research_report/`
