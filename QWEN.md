...
- Skill: `~/.qwen/skills/webcrawl/SKILL.md`
...
- `~/.qwen/settings.json` / `~/.mcpproxy/mcp_config.json`：純結構
...
- `~/.qwen/settings.json` 4 處 cmux hook 全指向 wrapper（Notification / SessionStart / Stop / UserPromptSubmit）
...
---
browser agent 定義（`~/.claude/agents/browser.md`）必須使用 camoufox-cli 作為主要工具，不可使用 playwright-cli 作為預設。
...
`~/.qwen/settings.json` 的所有 hook 都呼叫 `~/.claude/hooks/hook-dispatcher`，
...
**但每個同名 handler 的邏輯 parity 沒有逐個實測**。意即「Go 接管成功」是基於「~/.qwen/settings.json 全部指向 Go binary、Python 不在執行路徑上」的事實，不是「每個 handler 行為對等驗證通過」的結論。後者要 mock event payload 跑 Python 跟 Go 比 output 才能下定論，目前還沒做。
...
### 通用診斷模式
**Why:** 服務退場（archive / OSS 拆分）時，要同步更新的不只是 git tracked 的 registry，還有**用戶級 dotfile**（`~/.mcpproxy/mcp_config.json`、`~/.qwen/settings.json`、`~/.mcp.json` 等）。它們不在 git 範圍內，容易漏改。
...
`~/.venvs/omlx` 自 2026-03-10 建立時就用 brew Python 3.14 (`/opt/homebrew/opt/python@3.14/`) — 違反 `~/.qwen/QWEN.md` 全局禁令「`/opt/homebrew/bin/python3` is brew's 3.14, never use」。
...
| **3e** Worker mode skill | SKILL.md 加 Worker Mode Behavior 5 步驟（防重跑/執行/append seen/回報 done）| (~/.qwen/skills/session-channel/SKILL.md v0.2) |
...
- `~/.qwen/skills/session-channel/SKILL.md` v0.2.0 — worker mode + trust marker
...
```
~/.qwen/skills/web-video-tutorial/
├── templates/scripts/
...
一鍵用法：
```bash
bash ~/.qwen/skills/web-video-tutorial/templates/scripts/record-deployed.sh \
