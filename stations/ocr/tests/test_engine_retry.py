"""
Test adversary suite for _retry_with_backoff in OCR engines.
...
class RetryBehaviorMixin:
    """
    Shared test logic for all three OCR engine retry helpers.
    ...
    def test_all_fail_raises_exception(self):
        ...
        sentinel = ValueError("always broken")
    ...
    def test_all_fail_does_not_return_none(self):
        ...
        with patch("time.sleep"):
            ...
            try:
                result = self.retry_fn(fn, max_retries=3)
                # If we reach here the impl returned instead of raising
    ...
    def test_exponential_delay_default_params(self):
        ...
        attempt 0 succeeds: no sleep
        attempt 1 (after fail 0): sleep = min(1.0 * 2**0, 30) = 1.0
        attempt 2 (after fail 1): sleep = min(1.0 * 2**1, 30) = 2.0
    ...
    def test_delay_cap_enforced(self):
        ...
        Uncapped delays would be: 1, 2, 4, 8, ...
        Capped: [1, 2, 4, 4] (min, not max)
        """
    ...
    def test_delay_cap_distinguishes_min_from_max(self):
        ...
        sleep_args = [c.args[0] for c in mock_sleep.call_args_list]
        # delay[2] = attempt 2 → uncapped = 1 * 2**2 = 4.0 → capped = 3.0
        self.assertAlmostEqual(
            sleep_args[2],
            3.0,
            places=5,
...
class TestAllEnginesConsistent(unittest.TestCase):
    """Sanity: all three OCR engines expose identical retry semantics."""
