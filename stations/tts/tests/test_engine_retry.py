"""
Test suite for _retry_with_backoff inline helpers in TTS engines.
...
# ---------------------------------------------------------------------------
# Parametrize over both TTS engine implementations
# ---------------------------------------------------------------------------

IMPLEMENTATIONS = [
    pytest.param(_retry_with_backoff, id="apple"),
    pytest.param(_retry_with_backoff, id="elevenlabs_api"),
]
...
def test_success_first_call_invokes_fn_once(retry_fn):
    fn = _always_succeed("tts-result")
    with patch("time.sleep"):
        result = retry_fn(fn)
    fn.assert_called_once()
    assert result == "tts-result"
...
def test_permanent_failure_raises_last_exception(retry_fn):
    ...
    original_exc = KeyError("specific tts key")
...
def test_permanent_failure_all_exception_types_caught(retry_fn):
    ...
    for exc_type in [ValueError, TypeError, OSError, IOError, Exception]:
        fn = MagicMock(side_effect=exc_type("tts boom"))
        with patch("time.sleep"):
...
def test_fail_then_succeed_returns_value(retry_fn):
    fn = _fail_then_succeed(fail_count=2, return_value="tts-recovered")
    with patch("time.sleep"):
        result = retry_fn(fn, max_retries=3)
    assert result == "tts-recovered"
...
def test_fail_then_succeed_exactly_at_boundary(retry_fn):
    ...
    fn = _fail_then_succeed(fail_count=2, return_value="tts-boundary-ok")
    with patch("time.sleep"):
        result = retry_fn(fn, max_retries=3)
    assert result == "tts-boundary-ok"
    assert fn.call_count == 3
...
def test_fn_args_are_forwarded(retry_fn):
    """fn is called with no positional args (called as fn())."""
    fn = MagicMock(return_value="tts-x")
    with patch("time.sleep"):
        retry_fn(fn)
