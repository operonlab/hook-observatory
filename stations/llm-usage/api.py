...
Endpoints:
    ...
    GET /budget        — Monthly budget tracking (ccusage-based)
...
from ccusage_adapter import get_month_to_date

DEFAULT_CONFIG = SCRIPT_DIR / "config.json"

app = FastAPI(
    title="LLM Usage API",
    description="Dual-track LLM usage tracking — subscription + ccusage",
    version="2.1.0",
)
...
def summary_endpoint():
    """Dual-track merged summary: subscription + ccusage month-to-date."""
    config = _get_config()
...
@app.get("/budget")
def budget_endpoint():
    """Monthly budget tracking based on ccusage data."""
    config = _get_config()
    sub_cfg = config.get("subscriptions", {})

    # Monthly subscription total
    sub_total = sum(
        v.get("monthly_cost_usd", 0) for v in sub_cfg.values()
    )

    # ccusage month-to-date actual spend
    mtd = get_month_to_date()
    used_usd = mtd.get("total_cost_usd", 0)

    # Budget = subscription total (what we pay monthly)
    budget_usd = sub_total if sub_total > 0 else 300.0
    ...
    return {
        ...
        "used_usd": used_usd,
        "remaining_usd": round(budget_usd - used_usd, 2),
        "used_pct": pct,
...
@app.get("/cache")
def cache_endpoint():
    """Cache efficiency statistics from ccusage."""
    mtd = get_month_to_date()
    ...
    return {
        ...
        "cached_tokens": mtd.get("total_cache_read", 0),
        "total_input_tokens": (
            mtd.get("total_tokens_in", 0)
            + mtd.get("total_cache_creation", 0)
            + mtd.get("total_cache_read", 0)
        ),
